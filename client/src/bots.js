/* ── Bot definitions + message generation (client-side) ── */
...
export function generateBotMessage(isCoordination = false) {
...
export function generateInitialBurst(count = 15) {
  const items = [];
  for (let i = 0; i < count; i++) {
    const msg = generateBotMessage(Math.random() < 0.3);
